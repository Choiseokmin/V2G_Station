...
for i = 1:length(Q_Table.Row)
    for j = 1:3 % j는 소켓 갯수 수(0~4)
        if startsWith(cell2mat(Q_Table.Row(i)),"L")
            if endsWith(cell2mat(Q_Table.Row(i)),"0")
                Init_Reward = max((50 - 80*(j-1)),0)*80 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"1")
                Init_Reward = max((150 - 80*(j-1)),0)*80 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"2")
                Init_Reward = max((250 - 80*(j-1)),0)*80 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            end
        elseif startsWith(cell2mat(Q_Table.Row(i)),"M")
            if endsWith(cell2mat(Q_Table.Row(i)),"0")
                Init_Reward = max((50 - 80*(j-1)),0)*130 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"1")
                Init_Reward = max((150 - 80*(j-1)),0)*130 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"2")
                Init_Reward = max((250 - 80*(j-1)),0)*130 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            end
        elseif startsWith(cell2mat(Q_Table.Row(i)),"H")
            if endsWith(cell2mat(Q_Table.Row(i)),"0")
                Init_Reward = max((50 - 80*(j-1)),0)*200 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+(alpha*(Init_Reward)));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"1")
                Init_Reward = max((150 - 80*(j-1)),0)*200 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+(alpha*(Init_Reward)));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"2")
                Init_Reward = max((250 - 80*(j-1)),0)*200 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            end
        end
    end
