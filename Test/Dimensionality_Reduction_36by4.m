...
for m = 1:m
    if Gaussian_Grid(m)<75
        Level(m) = "0";
    elseif Gaussian_Grid(m)>=75 && Gaussian_Grid(m)<150
        Level(m) = "1";
    elseif Gaussian_Grid(m)>=150 && Gaussian_Grid(m)<225
        Level(m) = "2";
    elseif Gaussian_Grid(m)>=225 && Gaussian_Grid(m)<300
        Level(m) = "3";
    end
end
...
Action_varTypes = {'double','double','double','double'};        %열 var의 type
Action_varNames = {'action 0', 'action 1', 'action 2','action 3'}; %열 이름
Action_sz = [length(Q_State) length(Action_varNames)];                 %21 by 4 행렬
...
for i = 1:length(Q_Table.Row)
    for j = 1:4 % j는 소켓 갯수 수(0~4)
        if startsWith(cell2mat(Q_Table.Row(i)),"L")
            if endsWith(cell2mat(Q_Table.Row(i)),"0")
                Init_Reward = max((37 - 80*(j-1)),0)*80 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"1")
                Init_Reward = max((112 - 80*(j-1)),0)*80 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"2")
                Init_Reward = max((187 - 80*(j-1)),0)*80 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"3")
                Init_Reward = max((267 - 80*(j-1)),0)*80 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            end
        elseif startsWith(cell2mat(Q_Table.Row(i)),"M")
            if endsWith(cell2mat(Q_Table.Row(i)),"0")
                Init_Reward = max((37 - 80*(j-1)),0)*130 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"1")
                Init_Reward = max((112 - 80*(j-1)),0)*130 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"2")
                Init_Reward = max((187 - 80*(j-1)),0)*130 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"3")
                Init_Reward = max((267 - 80*(j-1)),0)*130 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            end
        elseif startsWith(cell2mat(Q_Table.Row(i)),"H")
            if endsWith(cell2mat(Q_Table.Row(i)),"0")
                Init_Reward = max((37 - 80*(j-1)),0)*200 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+(alpha*(Init_Reward)));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"1")
                Init_Reward = max((112 - 80*(j-1)),0)*200 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+(alpha*(Init_Reward)));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"2")
                Init_Reward = max((187 - 80*(j-1)),0)*200 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            elseif endsWith(cell2mat(Q_Table.Row(i)),"3")
                Init_Reward = max((267 - 80*(j-1)),0)*200 + 80*(j-1)*Vehicle_Cost;
                Q_Table(i,j) = num2cell((1-alpha)*table2array(Q_Table(i,j))+alpha*(Init_Reward));
            end
        end
    end
                ...
                % for l = 1:3
                    if rand < (1-epsilon) % 1-입실론 그리디에 의한 액션
                        ...
                        if length(Minus_Epsilon_Action) == 1 %현재 스테이트의 최솟값이 하나일 때
                            ...
                            Reward = ((max((LevelTable.Gaussian_Grid(i)-(80*Action(K))),0)*LevelTable.State_Cost(i)) + ((80*Action(K))*Vehicle_Cost));
