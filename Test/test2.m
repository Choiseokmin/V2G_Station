...
for i = 1:m
    ...
    elseif LT(i).startsWith(pattern2) == 1
    ...
    elseif LT(i).startsWith(pattern3) == 1
...
for m = 1:m
    if Gaussian_Grid(m)>=0 && Gaussian_Grid(m)<100
        Level(m) = "0";
...
%     elseif Gaussian_Grid(m)>=100 && Gaussian_Grid(m)<200
%         State_Level(2) = "1";
%     elseif Gaussian_Grid(m)>=200 && Gaussian_Grid(m)<200
%         State_Level(3) = "2";
%     end
% end
% 
...
for p = 1:length(Time)
    for q = 1:length(State_Level)
        Q_State(p,q) = append(Time(p),State_Level(q));
    end
end

Q_State = reshape(Q_State.' , length(Time)*length(State_Level),1);
...
for i = 1:fix(length(Reward_List)/24)-1
    Moving_Average(i,1) = sum(Year_Reward(i:i+59))/60;
    if i + 60 == fix(length(Reward_List)/24)-1
        break;
    end
end
...
% plot(B,'o')
for i = 1:fix(length(Random_Reward_List)/24)-1
    Random_Moving_Average(i,1) = sum(random_year_reward(i:i+59))/60;
    if i + 60 == fix(length(Random_Reward_List)/24)-1
        break;
    end
end
...
plot(Year_Reward,'r')
hold on
plot(Moving_Average,'b')
hold on
plot(Random_Moving_Average,'m')
